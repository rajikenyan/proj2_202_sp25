...
class TestFunctionSignatures(unittest.TestCase):
    ...
    def test_read_csv_lines_type(self):
        ...
        self.assertTrue(listlen(result) == 10)
        answer = filter_rows(result, 'country', 'equal', 'Canada')
        self.assertTrue(listlen(answer) == 1)
        ans = filter_rows(result, 'year', 'greater_than', 2000)
        self.assertTrue(listlen(ans) == 6)
        res = filter_rows(result, 'electricity_and_heat_co2_emissions', 'less_than', 1.0)
        self.assertTrue(listlen(res) == 5) 
        res = filter_rows(result, 'total_co2_emissions_excluding_lucf_per_capita', 'equal', 5000.0)
        self.assertFalse(res)
        res = filter_rows(result, 'hello', 'less_than', 50.0)
        self.assertFalse(res)
        res = filter_rows(result, 'year', 'bigger_than', 2190)
        self.assertFalse(res)
        res = filter_rows(result, 'year', 'greater_than', 2.0)
        self.assertFalse(res)
